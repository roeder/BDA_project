...
Sampling takes place with `rstanarm`'s default settings: four chains with 4,000 samples, 2,000 of which are warmup samples.
...
We can also asses convergence by inspecting the trace plots of the sampling chains:
```{r load_bayesplot}
library(bayesplot)
theme_set(custom_ggtheme)
```

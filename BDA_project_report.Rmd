...
Drawing posterior predicitve samples allows us to check if the difference in the wage gap holds up when looking at relative values rather than absolute ones.
...
Sampling takes place with `rstanarm`'s default settings: four chains with 2,000 samples, 1,000 of which are warmup samples.
...
We can then plot the estimated shape parameters *k* of the generalised Pareto distribution in order to check the reliability of the LOO expected log pointwise predictive (`elpd`) estimate:
...
All *k* values are below 0.5, indicating quick convergence and solid predicite performance [^5].
...
As unexciting as it may be, this happenstance actually makes it easier to provide a summary of the predicted wage ratios for workers of different ages by simply taking the mean across all six panels:
...
Compared to the linear model, there are fewer divergent transitions (15), but of course it would be preferrable to see even fewer of them.
...
While there are endless options for modifying the model looking for an improvement, the few we have tried (flat priors for intercept and smooth function) did not improve the model fit.
...
While the linear model showed an increase in the wage gap toward higer ages, but no differences between the panels, we have the opposite problem with the GAMM:

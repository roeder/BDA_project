...
# (variable FOD1P) is present. Save into data frame as us16_filtered.
# Also download github_majors_list.csv

require(dplyr) 

majors_list <- read.csv("github_majors_list.csv",header=TRUE,stringsAsFactors=FALSE)

working <- merge(us16_filtered,majors_list,by="FOD1P") # Check merge properly
us16_filtered  <- working
rm(working)

# Create dummy for full-time, year-round workers
us16_filtered$FTYR <- 0
us16_filtered$FTYR[us16_filtered$WKW == 1 & us16_filtered$WKHP >= 35] <- 1
nrow(us16_filtered[us16_filtered$FTYR==1,])

# If you want inflation-adjusted earnings, this is how to do it
# MAJOR1012 <- mutate(us16_filtered,RealEarn = PERNP * (ADJINC/1000000))
...
allage <- filter(us16_filtered,SCHL == 21) # No grad degree earners
allage.ftyr <- filter(us16_filtered,SCHL == 21 & FTYR == 1 & PERNP >0)
over25.nograd <- filter(us16_filtered,AGEP > 24 & SCHL == 21)
over25.grad <- filter(us16_filtered,AGEP > 24 & SCHL > 21)
young <- filter(us16_filtered,AGEP < 28) # This DOES include students with grad degrees
...
AllAges <- data.frame(FOD1P = as.numeric(majors_list$FOD1P[2:nrow(majors_list)]),Major = majors_list$Major[2:nrow(majors_list)],Major_Cat = majors_list$Major_Category[2:nrow(majors_list)],
                      Total = NA, Employed = NA, FTYR = NA, Unemployed = NA, Urate = NA, Median = NA, P25th = NA, P75th = NA)
Ages25up <- data.frame(FOD1P = as.numeric(majors_list$FOD1P[2:nrow(majors_list)]),Major = majors_list$Major[2:nrow(majors_list)],Major_Cat = majors_list$Major_Category[2:nrow(majors_list)],
...
Young <- data.frame(FOD1P = as.numeric(majors_list$FOD1P[2:nrow(majors_list)]),Major = majors_list$Major[2:nrow(majors_list)],Major_Cat = majors_list$Major_Category[2:nrow(majors_list)],
